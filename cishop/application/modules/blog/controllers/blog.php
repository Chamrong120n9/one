...
function _draw_feed_hp(){
    ...
    $mysql_query="select * from blog order by date_published desc limit 0,5";
...
function delete_image($update_id)
{
    ...
    $big_pic_path='./blog_pic/'.$picture;
    $small_picture=str_replace('.', '_thumb.', $picture);
    $small_pic_path='./blog_pic/'.$small_picture;;
...
function _generate_thumbnail($file_name,$thumbnail_name){
    ...
    $config['source_image'] = './blog_pic/'.$file_name;
    $config['new_image'] = './blog_pic/'.$thumbnail_name;
...
function delete($update_id){
    ...
    if($submit=="Cancel"){
        redirect('blog/create/'.$update_id);
         ...
         $flash_msg="The Blog Entry Have been Delete";
        $value='<div class="alert alert-success" role="alert">'.$flash_msg.'</div>';
        $this->session->set_flashdata('item',$value);
        ...
        $this->form_validation->set_rules('page_content','Blog Entry Content','required');
         ...
         if($this->form_validation->run()==TRUE){
            ...
            if (is_numeric($update_id)) {
                ...
                $flash_msg="This Blog Entry was successfully update.";
                $value='<div class="alert alert-success" role="alert">'.$flash_msg.'</div>';
                $this->session->set_flashdata('item',$value);
                redirect('blog/create/'.$update_id);
            }else{
                ...
                $this->_insert($data);
                $update_id=$this->get_max(); //get the ID of the new Blog Entry
                
                $flash_msg="This Blog Entry was successfully create.";
                $value='<div class="alert alert-success" role="alert">'.$flash_msg.'</div>';
                $this->session->set_flashdata('item',$value);
                redirect('blog/create/'.$update_id);
            }
